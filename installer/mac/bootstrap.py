#!/usr/bin/env python3
"""OpenSquawk Bridge — updater + environment builder + app launcher.

Run by the launcher via `uv run --python 3.12 --no-project bootstrap.py`.
...
Responsibilities, in order:
  ...
  4. Launch bridge_app.py from `src/`.
...
def ensure_env() -> None:
    ...
    subprocess.run([str(UV), "venv", str(VENV), "--python", PYTHON],
                   check=True)
    req = SRC / "requirements.txt"
    if req.exists():
        subprocess.run(
            [str(UV), "pip", "install", "--python", str(VENV_PY),
             "-r", str(req)],
            check=True)
...
def launch_app() -> None:
    ...
    if DRY_RUN:
        ...
        return
    os.chdir(SRC)
    os.execv(str(VENV_PY), [str(VENV_PY), str(entry)])
